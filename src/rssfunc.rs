...
pub async fn get_feed(url: &str,limit:usize)-> Result<(),Box<dyn Error>>{
...
pub async fn add_feed(url:&str)->Result<(), Box<dyn Error>>{
    //validate  the  URL format
    if !url.starts_with("http://") && !url.starts_with("https://"){
        return Err("Invalid URL: must start with http:// or https://".into());
    } 
...
pub async fn read_feed(id:usize)->Result<(), Box<dyn Error>>{
...
pub async fn read_multiple_feeds(limit:usize)->Result<(), Box<dyn Error>>{
